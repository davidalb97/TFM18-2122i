...
class StochasticDescentApproach(MyBaseRegressor):
    ...
    def get_algorithm_type(self) -> AlgorithmType:
        ...
        algorithm_name: str = "%s (Stochastic descent)" % algorithm_name
        return list(filter(lambda x: algorithm_name == x.value[0], AlgorithmType))[0]

    def get_model(self) -> Any:
        if not self.source_algorithm.get_algorithm_type().value[3]:
            raise Exception("Cannot get machine learning model from non machine learning source algorithm")
        # noinspection PyUnresolvedReferences
        return self.source_algorithm.get_model(self)
    ...
    def __predict_from_source(self, source_prediction: float) -> float:
        ...
        if delta > -threshold:  # and bool(random.getrandbits(1)):
            # stochastic_multiplier = random.random()
            stochastic_multiplier = random.randint(0, 25)
            curr_eRange = self.prev_eRange + int(delta * 0.01 * stochastic_multiplier)
        else:
    ...
    def predict(
        ...
        if is_source_algorithm_a_ml_algorithm:
            source_ml_algorithm: MyBaseRegressor = self.__get_source_ml_algorithm()
            return source_ml_algorithm.predict(prediction_input=prediction_input)
        else:
