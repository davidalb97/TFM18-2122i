...
class MyBayesianRidgeRegression(MyBaseRegressor):

    __bayesian_ridge_regressor_model: linear_model.BayesianRidge

    def __init__(self):
        self.__bayesian_ridge_regressor_model = linear_model.BayesianRidge()

    def get_algorithm_type(self) -> AlgorithmType:
        return AlgorithmType.ML_BAYESIAN_RIDGE_REGRESSION

    def get_model(self) -> Any:
        return self.__bayesian_ridge_regressor_model
    ...
    def learn_from_dataframes(self, input_dataframe: DataFrame, expected_output_dataframe: DataFrame):
        ...
        self.__bayesian_ridge_regressor_model.fit(input_output_numpy_array, expected_output_numpy_array)
    ...
    def predict_from_dataframe(self, input_dataframe: DataFrame) -> float:
        ...
        input_output_numpy_array: numpy.ndarray = input_dataframe.to_numpy()
        return self.__bayesian_ridge_regressor_model.predict(input_output_numpy_array)[0]
