...
class BaseAlgorithmEvaluation:
    ...
    def evaluate(self, expected: list[float], result: list[float], variable_count: int) -> float:
        len_expected = len(expected)
        len_result = len(result)
        if len_expected != len_result:
            raise Exception("Both list should have the same value! %d!=%d" % (len_expected, len_result))
        return self._evaluate(expected=expected, result=result, variable_count=variable_count)
